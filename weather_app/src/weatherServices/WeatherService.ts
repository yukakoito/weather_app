...
export const getWeatherData = async(locationId: number): Promise<Weather> => {
  const currentWeather = await fetch(`${url}weather?id=${locationId}&appid=${key}&units=metric`);
...
export const getForecast = async(locationId: number): Promise<Weather[]> => {
  const forecast = await fetch(`${url}forecast?id=${locationId}&appid=${key}&units=metric&cnt=8`);
